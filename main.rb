...
class SuiteBackend
  ...
  def initialize(state_dir: File.expand_path("~/.local/state/omarchy-layer-atlas"), runner: nil)
    @state_dir = state_dir
  ...
  def scan_system
    data = parse_json(command_text(["hyprctl", "layers", "-j"])) || {}
    rows = []
    data.each do |monitor, info|
      levels = info.is_a?(Hash) ? info["levels"] : nil
      next unless levels.is_a?(Hash)
    ...
    @score = rows.length
    @summary = rows.empty? ? "No surfaces" : "#{rows.length} surfaces"
